...
def read_config():
    ...
    PATH_stats_file_folder = config.get(
        'Folders/Files', 'PATH_stats_file_folder')
    PATH_bookkeeping_proj_folder = config.get(
        'Folders/Files', 'PATH_bookkeeping_proj_folder')
    PATH_tokens_file_folder = config.get(
        'Folders/Files', 'PATH_tokens_file_folder')
...
def tokenize_files(file_string, comment_inline_pattern, comment_open_close_pattern, separators):
    file_string_text = file_string.decode('utf-8')
    final_stats = 'ERROR'
    final_tokens = 'ERROR'
    ...
    lines = file_string_text.count('\n')
    print(lines)
    if not file_string_text.endswith('\n'):
        lines += 1
    file_string_text = "".join(
        [s for s in file_string_text.splitlines(True) if s.strip()])
   
    LOC = file_string_text.count('\n') 
    if not file_string_text.endswith('\n'):
        LOC += 1
    re_time = dt.datetime.now()
    # Remove tagged comments
   
    file_string_text = re.sub(comment_open_close_pattern, '',
                        file_string_text, flags=re.DOTALL)
  
    # Remove end of line comments
    file_string_text = re.sub(comment_inline_pattern, '',
                         file_string_text, flags=re.MULTILINE)
    re_time = (dt.datetime.now() - re_time).microseconds

    file_string_text = "".join(
        [s for s in file_string_text.splitlines(True) if s.strip()]).strip()

    SLOC = file_string_text.count('\n')
    if file_string_text != '' and not file_string_text.endswith('\n'):
        SLOC += 1

    final_stats = (file_hash, lines, LOC, SLOC)

    # Rather a copy of the file string here for tokenization
    file_string_for_tokenization = file_string_text

    # Transform separators into spaces (remove them)
    s_time = dt.datetime.now()
    for x in separators:
        file_string_for_tokenization = file_string_for_tokenization.replace(
            x, ' ')
    ...
    whole_file_string_for_tokenization = file_string_for_tokenization
    File_allTOkens = open("allTokens",'a')
    number = 0 
    print(whole_file_string_for_tokenization.__len__() ) 
    while number <  (whole_file_string_for_tokenization.__len__() -1) :
     File_allTOkens.writelines(whole_file_string_for_tokenization[number])
     File_allTOkens.write(' ')
     number = number +1 
    File_allTOkens.write('@@@')
    File_allTOkens.write(' ')
    File_allTOkens.close
   
    # Count occurrences
    file_string_for_tokenization = collections.Counter(
        file_string_for_tokenization)
...
def process_file_contents(file_string, proj_id, file_id, container_path,
    ...
    logging.info('Attempting to process_file_contents ' +
                 os.path.join(container_path, file_path))
# error
    global file_count
    file_count += 1

    (final_stats, final_tokens, file_parsing_times) = tokenize_files(
        file_string, comment_inline_pattern, comment_open_close_pattern, separators)
    ...
    File_allTOkens = open("allTokens",'a')
    File_allTOkens.write(proj_id)
    File_allTOkens.write(' ')
    File_allTOkens.write(str(file_id))
    File_allTOkens.write('\n')
    ww_time = dt.datetime.now()
    FILE_stats_file.write(','.join([proj_id, str(file_id), '\"'+file_path+'\"', '\"'+file_url +
                          '\"', '\"'+file_hash+'\"', file_bytes, str(lines), str(LOC), str(SLOC)]) + '\n')
    w_time = (dt.datetime.now() - ww_time).microseconds

    ww_time = dt.datetime.now()
    FILE_tokens_file.write(','.join([proj_id, str(file_id), str(
        tokens_count_total), str(tokens_count_unique), token_hash+tokens]) + '\n')
    w_time += (dt.datetime.now() - ww_time).microseconds

    logging.info('Successfully ran process_file_contents ' +
                 os.path.join(container_path, file_path))

    # [s_time, t_time, w_time, hash_time, re_time]
...
#                                 zip_time, file_time, string_time, tokens_time, write_time, hash_time, regex_time)
        ...
        logging.warning('Unable to open tar on <'+proj_id+',' +
                        proj_path+'> (process '+str(process_num)+')')
        logging.warning(e)
        return
        ...
        with zipfile.ZipFile(proj_path, 'r') as my_file:
            ...
            for file in my_file.infolist():
                ...
                try:
                    ...
                    logging.warning('Unable to open file (1) <'+os.path.join(
                        proj_path, file)+'> (process '+str(process_num)+')')
                    break
                zip_time += (dt.datetime.now() - z_time).microseconds

                if my_zip_file is None:
                    logging.warning('Unable to open file (2) <'+os.path.join(
                        proj_path, file)+'> (process '+str(process_num)+')')
                    break
...
#                                 zip_time, file_time, string_time, tokens_time, write_time, hash_time, regex_time)
        ...
        logging.warning('Unable to open zip on <'+proj_path +
                        '> (process '+str(process_num)+')')
        logging.warning(e)
        return
...
def process_one_project(process_num, proj_id, proj_path, base_file_id,
    ...
    if project_format == 'leidos':
        ...
        logging.info('Starting leidos project <'+proj_id+',' +
                     proj_path+'> (process '+str(process_num)+')')

        if not os.path.isdir(proj_path):
            logging.warning('Unable to open project <'+proj_id +
                            ','+proj_path+'> (process '+str(process_num)+')')
            return

        # Search for tar files with _code in them
        tar_files = [os.path.join(proj_path, f) for f in os.listdir(
            proj_path) if os.path.isfile(os.path.join(proj_path, f))]
        tar_files = [f for f in tar_files if '_code' in f]
        if(len(tar_files) != 1):
            logging.warning('Tar not found on <'+proj_id+',' +
                            proj_path+'> (process '+str(process_num)+')')
            times = [0, 0, 0, 0, 0, 0, 0]
        ...
        FILE_bookkeeping_proj.write(
            proj_id+',\"'+proj_path+'\",\"'+proj_url+'\"\n')

    if project_format in ['zip']:
        proj_url = 'NULL'

        logging.info('Starting zip project <'+proj_id+',' +
                     proj_path+'> (process '+str(process_num)+')')
    ...
    logging.info('Project finished <%s,%s> (process %s)',
                 proj_id, proj_path, process_num)
...
def process_projects(process_num, list_projects, base_file_id, global_queue, project_format):
    ...
    file_handler = logging.FileHandler(os.path.join(
        PATH_logs, 'LOG-'+str(process_num)+'.log'))
    file_handler.setFormatter(logging.Formatter(FORMAT))
    logging.getLogger().addHandler(file_handler)

    FILE_files_stats_file = os.path.join(
        PATH_stats_file_folder, 'files-stats-'+str(process_num)+'.stats')
    FILE_bookkeeping_proj_name = os.path.join(
        PATH_bookkeeping_proj_folder, 'bookkeeping-proj-'+str(process_num)+'.projs')
    FILE_files_tokens_file = os.path.join(
        PATH_tokens_file_folder, 'files-tokens-'+str(process_num)+'.tokens')
...
def start_child(processes, global_queue, proj_paths, batch, project_format):
    ...
    print("here")
    print(proj_paths_list[:batch])
    paths_batch = proj_paths_list[:batch]
    del proj_paths_list[:batch]

    print("Starting new process %s" % (pid))
    p = Process(name='Process '+str(pid), target=process_projects, args=(pid,
                paths_batch, processes[pid][1], global_queue, project_format, ))
    processes[pid][0] = p
    p.start()
...
if __name__ == '__main__':
    ...
    if FILE_priority_projects != None:
        ...
        prio_proj_paths = zip(range(init_proj_id, len(
            prio_proj_paths)+init_proj_id), prio_proj_paths)
    ...
    if project_format in ['zip']:
        ...
        proj_paths = zip(range(1, len(proj_paths)+1), proj_paths)
        proj_paths_list = list(proj_paths)
    ...
    if os.path.exists(PATH_stats_file_folder) or os.path.exists(PATH_bookkeeping_proj_folder) or os.path.exists(PATH_tokens_file_folder) or os.path.exists(PATH_logs):
        print('ERROR - Folder ['+PATH_stats_file_folder+'] or ['+PATH_bookkeeping_proj_folder +
              '] or ['+PATH_tokens_file_folder+'] or ['+PATH_logs+'] already exists!')
        sys.exit(1)
    else:
    ...
    while len(prio_proj_paths) > 0:
        start_child(processes, global_queue,
                    prio_proj_paths, 1, project_format)

    # Start all other projects
    print("* Starting regular projects...")
    print()
    while len(proj_paths_list) > 0:
        start_child(processes, global_queue, proj_paths,
                    PROJECTS_BATCH, project_format)
